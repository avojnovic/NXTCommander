...
public class MINDdroid extends Activity implements BTConnectable, TextToSpeech.OnInitListener
{
	...
	/**
	 * Asks if bluetooth was switched on during the runtime of the app. For
	 * saving battery we switch it off when the app is terminated.
	 * 
	 ...
	 * 
	 * @param btOnByUs
	 *            true, when bluetooth was switched on by the app
	 */
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		thisActivity = this;
		mRobotType = this.getIntent().getIntExtra(
				SplashMenu.MINDDROID_ROBOT_TYPE, R.id.robot_type_1);
		setUpByType();
	...
	private void updateButtonsAndMenu()
	{
		...
		if (connected)
		{
			myMenu.add(0, MENU_TOGGLE_CONNECT, 1,
					getResources().getString(R.string.disconnect)).setIcon(
	...
	/**
	 * Method for performing the appropriate action when the ACTION button is
	 * pressed shortly.
	 */
	public void actionButtonPressed()
	{
		if (myBTCommunicator != null)
		{
			...
			{
				sendBTCmessage(BTCommunicator.NO_DELAY, BTCommunicator.DO_BEEP,
						392, 100);
			...
			switch (mRobotType)
			{
				...
				case R.id.robot_type_3:
					...
					for (int bite = 0; bite < 3; bite++)
					{
						...
						sendBTCmessage(bite * 400 + 200, motorAction, -75
								* directionAction, 0);
					}
	...
	/**
	 * Depending on the status (whether the program runs already) we stop it,
	 * wait and restart it again.
	 * 
	 * @param status
	 *            The current status, 0x00 means that the program is already
	 *            running.
	 */
		...
		public void handleMessage(Message myMessage)
		{
			switch (myMessage.getData().getInt("message"))
			{
				...
				case BTCommunicator.PROGRAM_NAME:
					if (myBTCommunicator != null)
					{
						byte[] returnMessage = myBTCommunicator
								.getReturnMessage();
				...
				case BTCommunicator.SAY_TEXT:
					if (myBTCommunicator != null)
					{
						byte[] textMessage = myBTCommunicator
								.getReturnMessage();
	...
	public void onInit(int status)
	{
		...
		if (status == TextToSpeech.SUCCESS)
		{
					...
					|| result == TextToSpeech.LANG_NOT_SUPPORTED)
			{
				...
				if (mRobotType == R.id.robot_type_4)
					showToast(R.string.tts_language_not_supported,
							Toast.LENGTH_LONG);
			}
		}
		else
		{
			...
			if (mRobotType == R.id.robot_type_4)
				showToast(R.string.tts_initialization_failure,
						Toast.LENGTH_LONG);
		}
	}
